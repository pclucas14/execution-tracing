...
from post_processing.utils import build_runtime_trace, find_paths, to_sequence, is_distinct_paths, WhereEntry, read_jsonl_file, StepLocation
...
if __name__ == '__main__':
    ...
    out = read_jsonl_file(args.trace_file)  # Assuming the first file contains the relevant data

    for (repo_info, test_traces) in out:

        start_nodes = []
        all_where_entries = []  # Collect all where entries from all trace files
        # repo_id = get_repo_url(image_name)

        breakpoint()
        for test_name, test_trace in test_traces:
            print(f'Processing trace for test: {test_name} in repo: {repo_info["repo"]})')
            metadata, raw_trace_data = test_trace['metadata'], test_trace['trace_data']
            ...
            filtered_nodes = [node for node in filtered_nodes if len(find_paths(node.root, node)) >= args.min_path_amt]
